...
export class ProductsAdminComponent implements OnInit, AfterViewInit {
  ...
  disable(product) {
    product.status = 99;
    this.commonService.put(links.products + '/' + product.id, product).subscribe(data => {
      this.getProductsList();
    });
  }
